...
namespace CUDLR {
    ...
    public class CUDLRCommandAttribute : Attribute
    {
        public delegate void CallbackSimple();
        public delegate void Callback(string[] args);


        public CUDLRCommandAttribute() : this(string.Empty)
        {
            
        }
        public CUDLRCommandAttribute(string cmd) : this(cmd, string.Empty)
        {
            
        }

        public CUDLRCommandAttribute(string cmd, string help, bool runOnMainThread = true)
        {
          m_command = cmd;
          m_help = help;
