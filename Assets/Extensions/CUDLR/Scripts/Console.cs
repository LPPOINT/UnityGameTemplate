...
namespace CUDLR {
  ...
  struct QueuedCommand {
    public CUDLRCommandAttribute command;
    public string[] args;
  }
  ...
  public class Console {
    ...
    public static void Update() {
      while (Instance.m_commandQueue.Count > 0) {
        QueuedCommand cmd = Instance.m_commandQueue.Dequeue();
          if(cmd.command == null || cmd.command.m_callback == null) continue;
    ...
    [CUDLRCommand("clear", "clears console output", false)]
    public static void Clear() {
      Instance.m_output.Clear();
    }

    /* Print a list of all console commands */
    [CUDLRCommand("help", "prints commands", false)]
    ...
    public static void RegisterCommand(string command, string desc, CUDLRCommandAttribute.Callback callback, bool runOnMainThread = true) {
      if (string.IsNullOrEmpty(command)) {
        ...
        else throw new Exception("Command String cannot be empty");
      }
      ...
      public static void RegisterSingletonCommands(object singleton)
      {
          var st = singleton.GetType();
          foreach (MethodInfo method in st.GetMethods())
          {
              var attrs = method.GetCustomAttributes(typeof(CUDLRCommandAttribute), true) as CUDLRCommandAttribute[];
              if (attrs.Length == 0)
                  continue;
              var a = attrs[0];
              var cb = Delegate.CreateDelegate(typeof(CUDLRCommandAttribute.Callback), singleton, method.Name, false, false) as CUDLRCommandAttribute.Callback;

              if (cb == null)
              {
                  var cbs = Delegate.CreateDelegate(typeof(CUDLRCommandAttribute.CallbackSimple), singleton, method.Name, false, false) as CUDLRCommandAttribute.CallbackSimple;
                  if (cbs != null)
                  {
                      cb = args => cbs();
                  }
              ...
              if (cb == null)
              {
                  Debug.LogError("Cant create singletor console callback");
                  return;
              }

              RegisterCommand(a.m_command, a.m_help, cb, a.m_runOnMainThread);
  ...
  class CommandTree {
    ...
    private CUDLRCommandAttribute m_command;
