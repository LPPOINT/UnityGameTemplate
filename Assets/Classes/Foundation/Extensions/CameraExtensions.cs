...
namespace Assets.Classes.Foundation.Extensions
{
    public static class CameraExtensions
    {
        ...
        public static Rect GetCameraViewRect(this Camera camera)
        {
            var topLeft = GetCameraPoint(camera, Alignment.TopLeft);
            var topRight = GetCameraPoint(camera, Alignment.TopRight);
            var bottomLeft = GetCameraPoint(camera, Alignment.BottomLeft);

            return new Rect(topLeft.x, topLeft.y, Math.Abs(topRight.x - topLeft.x), Math.Abs(bottomLeft.y - topLeft.y));
